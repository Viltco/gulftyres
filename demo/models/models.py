...
class GulfCostTyres(models.Model):
    ...
    pur_dt=fields.Datetime(string="Pur-dt", compute="compute_date_receipt")
    ...
    def compute_date_receipt(self):
        for i in self:
            ...
            for l in date:
                for j in l.move_ids_without_package:
    ...
    def compute_product_invoice_sale(self):
        for rec in self:
            ...
            for i in qty:
                if i.state == 'assigned' and i.picking_type_id.sequence_code == 'OUT':
    ...
    def compute_product_invoice_purchase(self):
        for rec in self:
            ...
            qty = self.env['stock.picking'].search([])
            for i in qty:
                if i.state == 'assigned' and i.picking_type_id.sequence_code == 'IN':
    ...
    def compute_sale_walk_in(self):
        for i in self:
            a = 0
            obj = self.env['product.pricelist'].search([('name', '=', 'Sale1')])
            for j in obj.item_ids:
                if j.product_tmpl_id.id == i.id:
                    a = j.fixed_price
                i.sale1 = a
    ...
    def compute_sale_credit(self):
        for i in self:
            a = 0
            obj = self.env['product.pricelist'].search([('name', '=', 'Sale2')])
            for j in obj.item_ids:
                if j.product_tmpl_id.id == i.id:
                    a = j.fixed_price
                i.sale2 = a
    ...
    def compute_sale_cash(self):
        for i in self:
            a = 0
            obj = self.env['product.pricelist'].search([('name', '=', 'Sale3')])
            for j in obj.item_ids:
                if j.product_tmpl_id.id == i.id:
                    a = j.fixed_price
                i.sale3 = a
