...
class GulfCostTyres(models.Model):
    ...
    def compute_product_invoice_sale(self):
        total = 0
        qty = self.env['stock.picking'].search([('state', '=', 'assigned')])
        if qty:
            for i in qty.move_line_ids_without_package:
                a = i.search([('product_id', '=', self.name)])
                if a:
                    total = i.product_uom_qty+total
                    self.p_inv = total

    def compute_product_invoice_purchase(self):
        total = 0
        qty2 = self.env['stock.picking'].search([('state', '=', 'assigned')])
        if qty2:
            for i in qty2.move_ids_without_package:
                b = i.search([('product_id', '=', self.name)])
                if b:
                    total = i.product_uom_qty + total
                    self.pur_qty = total


    def action_view_smart_reserve(self):
        for i in self:
            pass
